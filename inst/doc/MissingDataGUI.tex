...
Pairwise plots are drawn by the function \code{ggpairs} of package \pkg{GGally} \citep{ggally}. The variable names and scales lay on the diagonal. For the continous variables, the pairwise scatterplots are in the lower triangle while the correlation coefficients for both missing and non-missing values between every paired variables are shown in the upper triangle. For the categorical variables, barcharts by the categories of the vertical variable are given on both triangles. Again, the bars are colored in proportion to the missings. The combination of continuous and categorical variables is displayed as side-by-side boxplots of missing and non-missing values for each category on the upper triangle and side-by-side histograms on the lower triangle. Limited by the room of the graphics device in the main window, the pairwise plots cannot contain too many variables. The upper limit of the number of variables is set to be 5 and the lower limit is apparently 2.
...
\begin{figure}[h]
\begin{centering}
\includegraphics[width=0.7\textwidth]{Miss_fig7}
\par\end{centering}
\caption{The data export GUI.}
\label{fig: export}
\end{figure}
\par\end{center}
...
\begin{figure}[h]
\begin{centering}
\includegraphics[width=0.9\textwidth]{Miss_fig8}
\par\end{centering}
...
\item The color by variable selector allows text entry to find the variable. This feature is very useful when there are a lot of variables with missing values.
...
\begin{figure}[h]
\begin{centering}
\includegraphics[width=0.6\textwidth]{Miss_fig9}
\par\end{centering}
...
This missing data GUI is written in \proglang{R} 2.15.2 \citep{r} and
based on the package \pkg{gWidgets} \citep{gwidgets} with the toolkit
RGtk2. 
