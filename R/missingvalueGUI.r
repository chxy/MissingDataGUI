...
##' @examples
##' 1+1
regularimpute = function(dat, method, vartype, cond) {
	...
	if (n!=1) {
		...
		if (sum(complete.cases(dat))!=0 & (!("character" %in% vartype))) {
			#dat=impute.knn(as.matrix(dat), colmax=0.9)$data
...
##' @examples
##' 1+1
myimpute = function(origdata, method, vartype, missingpct, condition=NULL){
	n=length(vartype)
	dat = origdata
	if (n==1){
		if (method %in% c('Regression','Nearest neighbor','Multiple Imputation')) {
			...
			if (is.null(condition)) {
				...
				if (!is.null(dat)) {
					Missing_any = factor(origshadow,levels=c(FALSE,TRUE),labels=c('Not','Missing'))
					res = data.frame(dat,origshadow,Missing_any)
				} else {res=NULL}
			} else {
				dat = ddply(origdata,condition,regularimpute,
		...
		if (is.null(condition)) {
			...
			if (method == 'Nearest neighbor') {
				if (sum(complete.cases(origdata))==0) {
					...
					if ("character" %in% vartype) {
						...
						#dat=impute.knn(as.matrix(origdata), colmax=0.9)$data
						myNNdat = dat[complete.cases(dat),]
			...
			if (!is.null(dat)) {
				Missing_any = factor(apply(origshadow,1,any),levels=c(FALSE,TRUE),labels=c('Not','Missing'))
				res = data.frame(dat, origshadow ,Missing_any)
			} else {res = NULL}
		} else {
			dat = ddply(origdata,condition,regularimpute, method=method,vartype=vartype, cond=condition)
			if (!is.null(dat)) {
				dat = dat[,(ncol(dat)-2*n+1):ncol(dat)]
				Missing_any = factor(apply(dat[,(n+1):(2*n)],1,any),levels=c(FALSE,TRUE),labels=c('Not','Missing'))
				res = data.frame(dat,Missing_any)
			} else {res = NULL}
		}
	}
...
##' @examples
##' 1+1
WatchMissingValues = function(h, data=NULL, ...){
    if (is.null(data)) {
...
#####------------------------------#####
    NumSmry = function(h,...) {
        ...
        NumSumforMisVal <- gwindow("Numeric Summary for Missing Values",
                                   visible = T, width = 350, height = 300)
...
#####----------------------------#####
    Graph = function(h, ...) {
        ...
        dat = myimpute(origdata=dataset[,c(gt11[name_select,2],cond)],
...
##  (2) Numeric summary.                                ##
##  (3) Plot the imputations.                           ##
##  (4) Export the imputations.                         ##
#####------------------------------------------------#####
